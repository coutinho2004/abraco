...
class Detalhes extends Funcoes implements IFuncoes {
	...
	public function setRazao_conta_corrente($razao_conta_corrente) {
		...
		if(is_numeric($razao_conta_corrente)) {
			...
			throw new Exception('Error: O campo Raz�o Conta Corrente não é um numero.');
		}
	}
	...
	public function setNome_pagador($nome_pagador) {
		...
		$nome_pagador = $this->montar_branco($this->retirarAcento($nome_pagador), 40, 'right');
	...
	public function setEndereco_pagador($endereco_pagador) {
		//	die($endereco_pagador);
			...
			if($tamanho > 40) {
				...
				$endereco_pagador = $this->montar_branco($this->retirarAcento($endereco_pagador), 40, 'right');
	 ...
	 * Cobrança, pois essa tarifa refere-se � negociada pelo Banco com seu cliente 
	 * benefici�rio. Orientação da FEBRABAN (Comunicado FB-170/2005).
	 * 
